...
class AugmentedPromptAgent:
    ...
    def respond(self, input_text):
        ...
        system_prompt = f"""
            You are to assume the role of the defined persona exactly as described below. 
            Forget any previous conversation, instructions, or context. 
            Do not rely on memory of earlier interactions — every session begins as if new.

            Persona: {self.persona}
            
            Rules:
            1. Only use the persona described above — do not deviate or reference prior conversations.
            2. If the user asks about past chats or history, state clearly that you do not retain memory.
            3. Stay in character consistently, answering only from the perspective of the defined persona.
        """
...
class EvaluationAgent:
    ...
    def evaluate(self, initial_prompt):
        ...
        iterations = 0
        system_prompt = f"""
        You are acting as an evaluator adopting this persona: {self.persona}.
        ...
        for i in range(self.max_interactions):# TODO: 2 - Set loop to iterate up to the maximum number of interactions:
            ...
            eval_prompt = (
                "You are an impartial evaluator. Judge ONLY on the criteria provided.\n"
            ...
            verdict_line = evaluation.splitlines()[0].lower() if evaluation else ""
            if "verdict:" in verdict_line and "yes" in verdict_line:
                print("✅ Final solution accepted.")
                break
            elif evaluation.lower().startswith("yes"):
                print("✅ Final solution accepted.")
                break
            else:
                print(" Step 4: Generate instructions to correct the response")
                instruction_prompt = (
                    "Generate minimal, actionable instructions to fix the answer so it meets ALL criteria. "
                    "Do NOT rewrite the entire answer; list only the corrections.\n\n"
                    f"Criteria:\n{self.evaluation_criteria}\n\n"
                    f"Current answer:\n{response_from_worker}\n\n"
                    f"Evaluator's reasons for failure:\n{evaluation}\n\n"
                    "Return a numbered list of specific changes."
                )
                response = client.chat.completions.create(
                    model="gpt-3.5-turbo",
                    temperature=0,
                    messages=[
                        {
                            "role": "system",
                            "content": (
                                "You are a precise reviewer. Provide only concrete corrections needed to satisfy the criteria."
                            ),
                        },
                        {
                            "role": "user",
                ...
                prompt_to_evaluate = (
                    ...
                    "It was evaluated as not meeting the criteria. "
                    "Revise your answer by applying ONLY the following corrections. "
                    "Do not introduce new claims; keep content valid.\n\n"
...
class RoutingAgent():
    ...
    def route(self, user_input): # TODO: 3 - Define a method to route user prompts to the appropriate agent
